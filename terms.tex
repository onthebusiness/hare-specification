...
which some data expects to be stored. The size of an object must be a multiple
of its alignment.

\example{An object with an alignment of 8 may be stored at addresses 8, 16, 32,
and so on; but not at address 4, unless the implementation supports unaligned
memory accesses.}

\specterm{character} a single Unicode codepoint encoded in the \hbox{UTF-8}
format.
...
this specification nor by the implementation.
