...
\specsubitem
Each identifier shall have an associated \nonterminal{symbol}. The symbol for a
\nonterminal{name} shall be itself, and the symbol for a \nonterminal{name}
within a namespace shall be the symbol for the namespace, followed by a '.',
followed by the \nonterminal{name}. The implementation may define the maximum
length of a \nonterminal{symbol}.
