...
Integer constants represent an integer value.
...
the type shall be \terminal{iconst} with both the maximum value and minimum
value set to the number provided.

\specsubitem
If the number provided is not representable within the type, or if the number
is not representable as an \terminal{iconst}, a diagnostic message shall be
...
of \nonterminal{unary-expression} is \terminal{iconst}, the result type shall be
an \terminal{iconst} whose minimum value is the result of negating the operand
type's maximum value, and whose maximum value is the result of negating the
operand type's minimum value. If the resulting \terminal{iconst} type isn't
representable, a diagnostic message shall be printed and the translation
environment shall abort. Otherwise, if the type of
\nonterminal{unary-expression} isn't \terminal{iconst}, the result type shall be
...
a flexible constant type, it shall first be lowered to its default type.
