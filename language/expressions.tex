...
initializer, then allocate an array of that capacity and set its first $N$
values to the $N$th value of the initializer, for each value of $N$ between $0$
...
slice or array, and the \nonterminal{simple-expression} shall have an integer
result type.
...
\specsubsubitem
All cast expressions are only valid for specific combinations of \textit{source
types} and \textit{destination types} which are specified by the following
table:
...
Function types & Cannot be cast \\
\terminal{bool}, \terminal{void} & Cannot be cast \\
...
storage semantics of types in the translation environment.
