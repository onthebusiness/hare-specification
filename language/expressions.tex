...
\specsubsubitem
In the first two forms, if \terminal{static} is omitted, the first
\nonterminal{expression} shall be evaluated in the execution environment. If the
expression evalutes to false, a diagnostic message shall be printed and the
execution phase aborted. The expression shall have type \terminal{bool}, which
...
\specsubsubitem
In the \terminal{abort} form, if \terminal{static} is omitted, the execution
environment shall unconditionally print a diagostic message and abort.

\specsubsubitem
If \terminal{static} is given, behavior is identical to when \terminal{static}
is omitted, except that the assertion is run in the translation environment
...
\nonterminal{type}. If \nonterminal{type} is a type of undefined alignment of a
...
\nonterminal{object-selector} shall either be a slice type, or a non-nullable
...
"va\_arg" macro. The result type of this expression is \terminal{void}.
...
\specsubsubitem
The \nonterminal{cast-expression} being acted upon must not be a terminating
expression.
...
Array types & Any array or slice type \\
Slice types & Any slice type or any pointer to an array type \\
...
\terminal{str}, \terminal{bool}, \terminal{rune}, or a pointer type.
...
\specsubsubitem
If any name in \nonterminal{binding-names} is \terminal{\_} (an underscore), a
...
be of a non-\terminal{const} type.
...
\specsubsubitem
A \nonterminal{defer-expression} shall not appear as a descendant of the
...
\nonterminal{expression} to terminate in a specific way. All control expressions
are terminating expressions, and their result type is \terminal{void}.
...
\nonterminal{compound-expression}. If an expression is provided, it shall not
...
\specsubsubitem
In any of these cases, if a \nonterminal{compound-expression} is selected, the
