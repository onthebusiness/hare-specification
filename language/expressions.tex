...
\specsubsubitem
If the types of the \terminal{==} or \terminal{!=} operators are numeric, they
shall be subject to type promotion. Otherwise, each operand must be of the same
type.
...
\nonterminaldef{logical-and-expression} \\
	\nonterminal{comparision-expression} \\
...
first term to be assigned the value given by the second term. The result type
of the first term shall not be of a \terminal{const} type.
...
\specsubsubitem
In the second form, the \nonterminal{postfix-expression} shall be of a
non-nullable pointer type, and the object which is assigned shall be the
secondary object to which the pointer object refers. The second term shall be
assignable to the pointer's secondary type.
