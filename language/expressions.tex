...
\nonterminal{array-members} shall receive this member type as a type hint.
...
described by the types of its \nonterminal{complex-expressions} in the order
that they appear. If a type hint is available, the tuple items may be assigned
to the types of the respective tuple sub-types in the order that they appear.
The implementation shall cause the side-effects of each
\nonterminal{complex-expression} to occur in the order that they appear.
...
\specsubsubitem
In the \terminal{alloc} form, the \nonterminal{simple-expression} is the
initializer for a new object whose type shall be inferred from the type hint.
If this type is a slice, the result type of \terminal{alloc} shall be that
slice type, and the initializer shall be provided that slice type as a type
hint. If the type hint is a pointer type, the result type shall be that type,
and the initializer provided the pointer type's secondary as its type hint. In
the case of any other type hint, or no type hint, the result type shall be a
pointer type whose secondary is the result type of the initializer, and the
initializer shall not be given a type hint.
...
pointer's referent type shall have a defined, nonzero size.
...
\nonterminal{simple-expression}'s result type shall be a mutable slice type.
This slice type's member type shall be the \terminal{append}'s
\textit{member type}. The result of an \terminal{append} shall be
\terminal{void}.

\specsubsubitem
The \nonterminal{simple-expression} of the first and third cases of
\nonterminal{append-values} shall have result types assignable to the
\terminal{append}'s member type. The \nonterminal{simple-expression} of the
second case of \nonterminal{append-values} shall have a result type assignable
to the result type of the \terminal{append}'s \nonterminal{simple-expression}.
...
\nonterminal{append-values} shall be appended to the slice object in the
order in which they appear. If the second case of the
\nonterminal{append-values} is specified, each of its members shall be appended
to the slice object in order.

\specsubsubitem
The side-effects of the \terminal{append}'s \nonterminal{simple-expression}
shall occur before the side-effects of the \nonterminal{append-values}. The
side-effects of the \nonterminal{append-values}'
\nonterminal{simple-expression}s shall occur in the order in which they appear.
...
\terminal{bool}, which shall be provided to it as a type hint.
...
\terminal{void} if no non-error types remain. If the type is a tagged union,
the result is a tagged union whose member types are the subset of the original
type which do not include the error flag; or, if there is only one such type,
that type without a tagged union; or, if there are no such types,
\terminal{void}. If the type is not a tagged union, but has the error flag set,
the result type of the error propagation is \terminal{void}. If there are no
types with the error flag among the constituent types of the
\nonterminal{postfix-expression}, the translation phase shall abort with a
diagnostic message.
...
\specsubsubitem
Except in the case of \terminal{str} to \terminal{*} \terminal{const}
\terminal{char}, the \terminal{const} flag shall not affect the rules for
casting one type to another. The same holds for the error flag as well.
...
\nonterminal{simple-expression} shall be \terminal{bool}. If a type hint is
provided, the \nonterminal{compound-expression} shall receive it as a type hint.
...
afterthought. The result type of the condition shall be \terminal{bool}, and
this shall be provided as a type hint.
...
\specsubsubitem
In the third form, the \nonterminal{complex-expression} shall be of a slice
...
\nonterminal{complex-expression} shall be assignable to this type. In this
second form, the type specified is used as a type hint for the
\nonterminal{complex-expression}.
...
evaluation of the \nonterminal{complex-expression}, and may be used in the
\nonterminal{complex-expression} \textit{only} as the object of the unary
addressing operator (\terminal{\&}).
...
the function's result type. It shall provide the result for the function. The
