...
\specsubsection{Literals}

\begin{grammar}
\nonterminaldef{literal} \\
...
\specsubsubitem
The \terminal{null} keyword represents the value of the null type.

\specsubsubitem
The \terminal{void} keyword represents the value of the \terminal{void} type.

\specsubsection{Floating literals}

\begin{grammar}
\nonterminaldef{floating-literal} \exactly \\
	\nonterminal{nonzero-decimal-digits} \terminal{.} \nonterminal{decimal-digits} \optional{\nonterminal{decimal-exponent}} \optional{\nonterminal{floating-suffix}} \\
	\nonterminal{nonzero-decimal-digits} \optional{\nonterminal{decimal-exponent}} \nonterminal{floating-suffix} \\
...
\nonterminaldef{nonzero-decimal-digits} \exactly \\
	\terminal{0} \\
	\nonterminal{nonzero-decimal-digit} \optional{\nonterminal{decimal-digits}} \\

\nonterminaldef{nonzero-decimal-digit}\oneof \\
\terminal{1} \terminal{2} \terminal{3} \terminal{4} \terminal{5}
...
\specsubitem
If the \nonterminal{floating-suffix} is not provided, the result shall be a
flexible float. Otherwise, the type shall refer to the type named by the suffix.

\specsubitem
\nonterminal{decimal-digits} shall be interpreted as base 10.
\nonterminal{hex-digits} shall be interpreted as base 16, in a case-insensitive
manner.
...
\nonterminaldef{positive-decimal-exponent} \\
	\nonterminal{decimal-exponent-char} \optional{\terminal{+}} \nonterminal{decimal-digits} \\
...
\specsubitem
\nonterminal{octal-digits} shall be interpreted as base 8.
\nonterminal{binary-digits} shall be interpreted as base 2.

\specsubitem
If the \nonterminal{decimal-exponent} is provided, the value of the
integer shall be multiplied by 10 to the power of \nonterminal{decimal-digits}.

\specsubsection{Rune literals}
\begin{grammar}
\nonterminaldef{rune-literal} \exactly \\
...
\specsubsubitem
The result type of a \nonterminal{rune-literal} shall be a flexible rune type.

\specsubsubitem
If the \nonterminal{rune-literal} is not an \nonterminal{escape-sequence}, the
value of the rune shall be the Unicode codepoint representing
\nonterminal{rune}.

\specsubsubitem
A \nonterminal{rune-literal} beginning with \terminal{\textbackslash{}x},
\terminal{\textbackslash{}u}, or \terminal{\textbackslash{}U} shall interpet its
value as a Unicode codepoint specified in its hexadecimal representation by
\nonterminal{hex-digit}s.

\specsubsubitem
A \nonterminal{rune-literal} containing a \nonterminal{named-escape} shall have
...
\specsubsection{String literals}

\begin{grammar}
\nonterminaldef{string-literal} \\
...
\specsubsubitem
A \nonterminal{string-literal} expression shall have a result type of
...
\subsubitemref{Rune literals}{17}.
...
\specsubsubitem
If a \nonterminal{string-literal} consists of more than one
...
result value "plainly". In the case of an \nonterminal{identifier}, the
...
\terminal{size} type. The execution environment shall choose a capacity equal to
...
\nonterminal{allocation-expression} shall be a slice whose data field refers to
...
\terminal{alloc} expression, freeing them for future use. If the
\nonterminal{expression}'s result type is a pointer type, and the result value
compares unequal to \terminal{null}, then the object referred to by the pointer
shall be freed. If the \nonterminal{expression}'s result type is a slice type or
\terminal{str}, then the array object referred to by its \code{data} field shall
be freed. Otherwise, the \nonterminal{expression}'s result value shall be
\terminal{null}, and no additional side-effects shall occur.
...
\nonterminal{type}. If \nonterminal{type} is a type of undefined alignment, a
...
member type of this slice shall be the \nonterminal{append-expression}'s
...
\nonterminal{append-expression}'s member type. The second term shall be provided
...
type of an \nonterminal{insert-expression} shall be \terminal{void}.
...
bounded array, a pointer to a non-expandable bounded array, or a slice. In all
cases, the secondary type of the array or slice type shall be the
\nonterminal{insert-expression}'s member type. The second term shall be provided
the object type as a type hint.
...
\specsubsubitem
If the \terminal{static} form is used, the slice object's capacity shall not be
modified, and its data shall not be reallocated. Otherwise, the execution
environment may decrease the slice object's capacity, re-allocating its data if
necessary. This re-allocation, if performed, shall not fail. The slice object
shall be updated to reflect the new capacity if necessary. If a
non-\terminal{static} \nonterminal{delete-expression} sets the value of the
slice object's \code{length} field to zero, the \code{capacity} field's value
shall also be set to zero, and the array object referred to by its \code{data}
field shall be freed.
...
of \nonterminal{unary-expression} is a flexible integer, the result type shall
...
\nonterminal{unary-expression} isn't a flexible integer, the result type shall
be equivalent to the type of \nonterminal{unary-expression}, which shall be of a
numeric type.
...
\secref{Flexible type promotion algorithm} shall be applied to them, and the
...
\specsubsubitem
Casting a pointer type to \terminal{uintptr}, and then back to the pointer
type, shall yield the same pointer. Likewise, casting the null type to a
\terminal{uintptr} and then back to a nullable pointer type shall yield
\terminal{null}.

\informative{However, casting \terminal{uintptr} to any other integer type
...
assertion with \terminal{null} as \nonterminal{assertion-target} shall be the
...
\specsubsubitem
Two \terminal{str} objects shall be equal if both strings have the same length
...
\specsubsubitem
If the condition uses the \secref{Translation compatible expression subset}, and
its result value is \terminal{true}, and the scope of the \nonterminal{for-loop}
is never terminated as a side-effect of a \terminal{break} or \terminal{yield}
expression, the result type of the \nonterminal{for-loop} shall be
\terminal{never}, otherwise, the result type shall be \terminal{void}.
...
\specsubsubitem
If a \nonterminal{label} is present, the established scope shall be labelled
with its \nonterminal{name}.
...
\terminal{str}, \terminal{bool}, \terminal{rune}, a flexible rune type, or a
pointer type.
...
\specsubsubitem
If the \nonterminal{switch-expression} has a \nonterminal{label}, its
\nonterminal{name} shall be used to label the scope of each case's implicitly
introduced \nonterminal{compound-expression}.
...
Forward references: \subsecref{Compound expressions}, \subsecref{Binding
expressions}
...
\specsubsubitem
If the \nonterminal{match-expression} has a \nonterminal{label}, its
\nonterminal{name} shall be used to label the scope of each case's implicitly
introduced \nonterminal{compound-expression}.
...
\nonterminaldef{assignment} \\
	\nonterminal{assignment-target} \nonterminal{assignment-op} \nonterminal{expression} \\

\nonterminaldef{assignment-target} \\
	\nonterminal{object-selector} \\
	\nonterminal{indirect-assignment-target} \\
	\nonterminal{slicing-assignment-target} \\

\nonterminaldef{indirect-assignment-target} \\
	\terminal{*} \nonterminal{unary-expression} \\
	\terminal{(} \nonterminal{indirect-assignment} \terminal{)} \\

\nonterminaldef{slicing-assignment-target} \\
	\nonterminal{slicing-expression} \\
	\terminal{(} \nonterminal{slicing-assignment-target} \terminal{)} \\
...
Forward references: \subsecref{Binding expressions}

\specsubsubitem
An \nonterminal{assignment} expression shall cause the object given by
\nonterminal{assignment-target} to be assigned a new value based on the value
...
term. Otherwise, the assignment \code{\nonterminal{e1}
\nonterminal{op}\terminal{=} \nonterminal{e1}} shall be equivalent to the
assignment \code{\nonterminal{e1} \terminal{=} \nonterminal{e1} \nonterminal{op}
(\nonterminal{e2})}, but the side-effects of \nonterminal{e1} shall only occur
once.

\specsubsubitem
In the \nonterminal{object-selector} form, the \nonterminal{object-selector}
selects the object to be modified. The type of this object shall not be a
\terminal{const} type.

\specsubsubitem
In the \nonterminal{indirect-assignment-target} form, the
\nonterminal{unary-expression} shall have a result of a non-nullable, non-const
...
\specsubsubitem
In the \nonterminal{slicing-assignment-target} form, the
\nonterminal{expression} shall be either of a slice type and have a length equal
to the slice given by \nonterminal{slicing-expression}, or an expandable array.
The \nonterminal{assignment-op} shall be \terminal{=}. The first term shall not
be of a \terminal{const} type, and the lengths of the two slices shall be equal.
...
\specsubsubitem
If at least one of the types is flexible, the
\secref{Flexible type promotion algorithm} shall be applied to them, and
...
\specsubsection{Binding expressions}
...
\nonterminal{expression}. The result type of a binding list expression is
\terminal{void}.

\specsubsubitem
A binding's \nonterminal{expression} shall be evaluated before any
\nonterminal{name}s are made available in the present scope.

\informative{Because the binding is only inserted into the scope when the
\nonterminal{binding-list} is evaluated, it isn't visible to any expressions
which lexically precede it, even if they reside in the same scope.}
...
the type of a binding which uses the \terminal{const} or \terminal{let} form is
a flexible type, it shall first be lowered to its default type.

\specsubsubitem
If the \terminal{static} form is used, the objects shall be allocated
...
\specsubsubitem
If the \terminal{def} form is used, the objects shall not have any storage
...
inserted if the second form of \nonterminal{binding} is used. The tuple
unpacking form of \nonterminal{binding} shall not be used with this form. The
\nonterminal{expression} associated with each binding shall be limited to
the \secref{Translation compatible expression subset}, and shall be evaluated in
the translation environment.

\specsubsubitem
If a given \nonterminal{name} already refers to an object visible in the present
scope, the new binding shall \textit{shadow} the earlier binding, causing any
references to the \nonterminal{name} which lexically follow this
\nonterminal{binding-list} within the present scope to resolve to the newly
bound object.
...
\specsubsubitem
The \nonterminal{expression} shall not have a result type whose error flag is
...
\informative{Thus, a binding created after a \nonterminal{defer-expression} is
evaluated can't be referenced within the deferred expression. This is also true
of shadowed bindings.}
...
\nonterminal{expression}s in sequence.
...
\specsubsubitem
The implementation shall establish a new scope for the
\nonterminal{compound-expression}. The expressions shall then shall be evaluated
...
\specsubsubitem
None of the items in the \nonterminal{expression-list} shall have a result type
...
expression or function. The result type is \terminal{never}.
...
associated scope's label doesn't match the provided \nonterminal{label} shall be
...
\specsubsubitem
In the \terminal{break} and \terminal{continue} forms, if a \nonterminal{label}
...
\nonterminal{for-loop}, a diagnostic message shall be printed and the
translation phase shall abort. The \terminal{break} form shall cause the loop to
end without evaluating the \textit{condition} or the \textit{afterthought}. The
\terminal{continue} form shall cause the loop to repeat immediately, running the
\textit{afterthought} if present, re-testing the \textit{condition}, and
repeating the loop if \terminal{true}.

\specsubsubitem
The \terminal{return} form shall select the current function. If an
\nonterminal{expression} is given, its result shall be used as the result of the
function, otherwise, \terminal{void} shall be used. The result type shall be
assignable to the function's result type, which shall be provided to the
\nonterminal{expression} as a type hint, if the \nonterminal{expression} is
present.

\specsubsubitem
In the \nonterminal{yield-expression} form, if a \nonterminal{label} isn't
provided, the selected expression shall be the \nonterminal{compound-expression}
whose associated scope encompasses the smallest region. If a \nonterminal{label}
is provided, the selected expression shall be the expression whose scope
encompasses the smallest region, and if this expression isn't a
\nonterminal{compound-expression}, a diagnostic message shall be printed and the
translation phase shall abort. A \nonterminal{yield-expression} that isn't
...
\informative{A \nonterminal{yield-expression} may select the implicit
\nonterminal{compound-expression} introduced by a
\nonterminal{switch-expression} or \nonterminal{match-expression}.}
...
determined by the \secref{Flexible type promotion algorithm}.
...
\terminal{uintptr}, expressions involving \terminal{uintptr} and the null type
...
\specsection{Flexible type promotion algorithm}
...
\specsubitem
If one operand is a flexible rune and the other is \terminal{rune}, the flexible
rune shall be lowered into a \terminal{rune}. If one operand is a flexible rune
