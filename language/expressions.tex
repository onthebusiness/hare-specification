...
\secref{Flexible constant promotion algorithm} applied to \terminal{fconst}
and the hint, or \terminal{fconst} if constant promotion fails. If the
\nonterminal{floating-suffix} is not provided and a type hint is not provided,
the type shall be \terminal{fconst}. Otherwise, the type shall refer to the type
named by the suffix.
...
number provided. If the \nonterminal{integer-suffix} isn't provided and a type
hint is provided, the \secref{Flexible constant promotion algorithm} shall be
aplied to the result type and the type hint. If promotion fails, the result
shall instead be the \terminal{iconst}'s default type.
...
\specsubsubitem
If a type hint is provided, the result type shall be the
\secref{Flexible constant promotion algorithm} applied to \terminal{rconst}
and the hint. If constant promotion fails or a hint is not provided, the result
type shall be \terminal{rconst}.
...
\nonterminaldef{string-constant} \\
	...
	\terminal{`} \optional{\nonterminal{rawstring-chars}} \terminal{`} \\
	\nonterminal{string-constant} \nonterminal{string-constant} \\
...
\nonterminaldef{rawstring-chars} \\
	\nonterminal{rawstring-char} \optional{\nonterminal{rawstring-char}} \\

\nonterminaldef{rawstring-char} \\
	\norm{Any character other than \terminal{`}} \\
\end{grammar}
...
\nonterminaldef{assertion-expression} \\
	\optional{\terminal{static}} \terminal{assert} \terminal{(} \nonterminal{expression} \terminal{)} \\
	\optional{\terminal{static}} \terminal{assert} \terminal{(} \nonterminal{expression} \terminal{,} \nonterminal{string-constant} \terminal{)} \\
	\optional{\terminal{static}} \terminal{abort} \terminal{(} \optional{\nonterminal{string-constant}} \terminal{)} \\
\end{grammar}
...
\nonterminal{object-selector}; then the length term, if present; then the
append values.

\specsubsubitem
The object selector shall be not be provided a type hint. If \terminal{...} is
...
a type hint of \terminal{size} if applicable.
...
from the \terminal{valist} object of the type provided by the type hint, and
...
must be of the \terminal{valist} type. The implementation shall finalize the
\terminal{valist} object in an implementation-defined manner which is
...
the type of \nonterminal{unary-expression}, which shall be of a numeric type.
...
In the former case, \nonterminal{type} shall be a constituent
type of that tagged union. The \nonterminal{cast-expression} shall be computed,
and if the tag does not match \nonterminal{type}, a diagnostic message shall be
printed and the execution environment shall abort. Otherwise, the result type
is \nonterminal{type}.

In the latter case \nonterminal{type} shall be a
non-nullable pointer type. The \nonterminal{cast-expression} shall be computed,
...
it was \terminal{false}, the \nonterminal{expression} shall be executed. The
...
shall be \terminal{void} and the if expression shall terminate. Otherwise, the
result type shall be the \secref{Result type reduction algorithm} applied to the
...
branches terminate, the result type shall be \terminal{void} and the switch
...
If a type hint is provided and all non-terminating branches are assignable to
that type, the result type shall be the type given by the type hint. If all
branches terminate, the result type shall be \terminal{void} and the match
expression shall terminate. Otherwise, the result type shall be the
\secref{Result type reduction algorithm} applied to the result types of the
non-terminating branches.
...
\nonterminal{yield-expression}s. In that case, the result type shall be the
\secref{Result type reduction algorithm} applied to the result types of the
yield expressions.
...
\specsubitem
An integer type may be promoted to an \terminal{enum} type whose storage is the
...
% XXX; Should ideally prevent assigning rconst => u8, but need to keep
% castability

\specsubitem
If one operand is \terminal{rconst} and the other is either an integer type or
\terminal{rune}, the \terminal{rconst} shall be lowered to the other type, and
