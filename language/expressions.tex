...
\nonterminaldef{allocation-expression} \\
	...
	\terminal{alloc} \terminal{(} \nonterminal{expression} \terminal{,} \nonterminal{expression} \terminal{)} \\
	\terminal{alloc} \terminal{(} \nonterminal{slice-items} \terminal{...} \terminal{,} \nonterminal{expression} \terminal{)} \\
	\terminal{free} \terminal{(} \nonterminal{expression} \terminal{)} \\

\nonterminaldef{slice-items} \\
	\nonterminal{expression} \\
	\nonterminal{slice-items} \terminal{,} \nonterminal{expression}
\end{grammar}
...
\textit{object type} of the allocation expression shall be based on its type
hint, which may be absent; if so, it shall be a pointer type whose secondary
type is the result type of the initializer.

\specsubsubitem
In the case of a pointer type as the object type, the pointer's referent type
shall have a defined, nonzero size. The execution environment shall provision
storage sufficient to store a value of the referent type, and provide the
pointer type's referent type as a type hint to the initializer. The new
object's value shall be set to the initializer result.

\specsubsubitem
In the case of a slice type as the object type, the execution environment shall
choose a capacity which shall be greater than or equal to the number of items
in the initializer, then provision an array of that length and set each $N$th
value to the $N$th value of the initializer, for each value of $N$ between $0$
(inclusive) and the length of the initializer (exclusive). The result of the
...
capacity is set to the allocated capacity.

\specsubsubitem
If the second form of \terminal{alloc} is used, the object type shall be a slice
type, and the second term shall be assignable to the \terminal{size} type. The
second term shall specify the capacity, which shall be greater to or equal to
the length of the initializer. The execution environment shall choose a
...
\specsubsubitem
If the third form of \terminal{alloc} is used, the object type shall be a slice
type, and the final term shall be assignable to the \terminal{size} type. The
execution environment shall choose a capacity equal to or greater than this
term, then provision an array of that length and set each $N$th value to the
$N$th value of \nonterminal{slice-items} (exclusive), for each value of $N$
between $0$ (inclusive) and the number of \nonterminal{slice-items}, then set
each $N$th value to the value of the last \nonterminal{slice-items} expression
for each subsequent value of $N$ up to the length given by the final term
(exclusive).  The result of the \nonterminal{allocation-expression} shall be a
slice whose data field refers to the provisioned array, and whose length is
equal to the final term, and whose capacity is set to the selected capacity.
...
\specsubsection{Type expressions}

\begin{grammar}
\nonterminaldef{type-expression} \\
	\terminal{type} \terminal{(} \nonterminal{type} \terminal{)}
\end{grammar}

\specsubsubitem
A \nonterminal{type-expression} shall have a result type of \terminal{type} and
returns a \terminal{type} value describing the given \nonterminal{type}.
...
Any pointer type & Any pointer type, \terminal{uintptr}, \terminal{null}, or \terminal{type} \\
...
\terminal{rune}, \terminal{type}, or a pointer type.
...
\specsubsubitem
If the type is \terminal{type}, an \nonterminal{equality-expression} shall be
\terminal{true} if both types have the same type ID, and \terminal{false}
otherwise.
