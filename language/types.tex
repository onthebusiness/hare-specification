...
is equal to the last value assigned to a member of this enum type plus one. If
no such previous value exists, the value is assigned as zero. If the implicitly
assigned value exceeds the precision of the underlying type, a diagnostic
message shall be printed and translation shall fail.

\specsubsubitem
If \nonterminal{simple-expression} is specified, it must use only features
included in the \subsecref{Translation compatible expression subset}. It shall
be evaluated in the translation environment and the resulting value shall be
assigned to that \nonterminal{enum-value}. The result type must be assignable to
the enum type's underlying integer type (ref \subsecref{Assignment expressions}).
