...
\specsubsubitem
If the \nonterminal{struct-union-type} form of \nonterminal{struct-field} is
given, it shall specify a struct type. The \nonterminal{struct-fields} of the
...
\nonterminaldef{prototype} \\
	\nonterminal{type} \\
	\terminal{(} \nonterminal{parameter-list} \optional{\nonterminal{variadism}} \optional{\terminal{,}} \terminal{)} \nonterminal{type} \\
...
\specsubsubitem
If the first form of \terminal{variadism} is usesd, the final parameter of the
function type uses \textit{Hare-style variadism}. If the second form is used,
the function uses \textit{C-style variadism}. The variadism of a function type
affects the calling semantics for that function.
