...
\item
The number of \nonterminal{enum-values}; the type with fewer values is
ordinally first.
\item
The value of each \nonterminal{enum-value} is compared between each enum type.
...
\informative{For example, in the type \code{(bool | int | uint)}, the sorted
list of constituent types would be \terminal{int}, \terminal{uint},
\terminal{bool}; which would be respectively assigned tags 0, 1, and 2.}
...
\informative{It follows that the types \code{(int | uint)} and \code{(uint | int)}
are equivalent.}
