...
\specsubsubitem
Enum types, or \textit{enumerated types}, are types which allow the user to
assign a name to specific values.

\specsubsubitem
If \nonterminal{enum-storage} is specified, the underlying storage of the enum
type is defined by the type specified. Otherwise, the underlying storage is
\terminal{int}.

\specsubsubitem
If an enum type's underlying storage is classified as a \textit{numeric type},
the encompassing enum shall also be considered a \textit{numeric type}.

\informative{In practice, this means that all enum types are
\textit{numeric types}, except for those whose underlying storage is
\terminal{rune}.}
