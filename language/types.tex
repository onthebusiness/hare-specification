...
\nonterminaldef{type} \\
	\nonterminal{type-const} \optional{\terminal{!}} \\

\nonterminaldef{type-const} \\
	\optional{\terminal{const}} \nonterminal{storage-class} \\
...
shall be provided the enum's type's underlying integer type as a type hint. The
result type must be assignable to the enum type's underlying integer type (ref
...
\informative{This allows the \nonterminal{simple-expression} for each value to
refer to previously declared values.}
...
\specsubsubitem
The \nonterminal{simple-expression} given for the \nonterminal{offset-specifier}
shall be limited to the \subsecref{Translation compatible expression subset},
and shall have an integer type and a positive or zero value.
...
than the offset of the previous value plus the size of the previous value. The
implementation shall add \textit{padding} to meet the alignment requirements of
tuple values.

\specsubsubitem
The size of a tuple is the sum of the sizes of its values plus any necessary
