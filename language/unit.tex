...
the value of the \nonterminal{expression} and shall have alignment greater than
or equal to the necessary alignment for the \nonterminal{type}. In the
\terminal{const} form, the \nonterminal{type}s shall have the constant flag
enabled by default.
...
by its \nonterminal{name}. Those which use the \terminal{\_} form shall not be
made available.
...
shall accept no parameters, shall return void, shall not appear in
\nonterminal{object-selector}s, need not have unique names, and shall not be
inserted into the unit's scope.

\specsubsubitem
Other semantics of \terminal{@test} functions are implementation-defined.
